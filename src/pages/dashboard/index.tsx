...
export function Dashboard() {
  ...
  async function handleSearch() {
    ...
    try {
      ...
      const productSnapshot = await getDocs(
        query(productRef, where("title", "==", productName?.trim()))
      );

      console.log(productSnapshot.docs[0].data() as ItemFoundProps);
      console.log("--------------");
  ...
  async function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (newCategory) {
      try {
        ...
        const existItem = categorySnapshot.docs.find(
          (doc) => doc.data().name === newCategory
        );
    ...
    try {
      ...
      toast.error(
        "Houve um problema ao registrar, tente novamente mais tarde!"
      );
      console.log(err);
    }
    ...
    try {
      ...
      const existentMosaic = mosaicQuery.docs.find(
  ...
  const handleNext = async () => {
    try {
      ...
      const nextFalseItemQuery = query(
        mosaicRef,
        where("status", "==", false),
        startAfter(lastTrueItem)
      );
  ...
  const handlePrev = async () => {
    try {
      ...
      const lastTrueItem =
        mosaicSnapshot.docs[mosaicSnapshot.docs.length - 1].ref;
      const firstTrueItem = mosaicSnapshot.docs[0];

      const prevFalseItemQuery = query(
        mosaicRef,
        where("status", "==", false),
        endBefore(firstTrueItem)
      );
      const prevFalseItemSnapshot = await getDocs(prevFalseItemQuery);

      if (!prevFalseItemSnapshot.empty) {
        const prevFalseItemRef =
          prevFalseItemSnapshot.docs[prevFalseItemSnapshot.docs.length - 1].ref;
  ...
  async function handleEdit() {
    ...
    try {
      ...
      const productDoc = productSnapshot.docs.find(
        (doc) => doc.data().title === productName
      );
  ...
  async function handleDelet() {
    try {
      ...
      const productsWithSameCategory = productSnapshot.docs.filter(
        (doc) => doc.data().category === itemCategoryRef
      );
      if (productsWithSameCategory.length === 1) {
        const mosaicRef = collection(db, "mosaic");
        const mosaicQuery = await getDocs(
          query(mosaicRef, where("category", "==", itemCategoryRef))
        );

        const categoryRef = collection(db, "categorias");
        const categoryQuery = await getDocs(
          query(categoryRef, where("name", "==", itemCategoryRef))
        );
      ...
      <Container>
        ...
        <div className="w-full max-w-900 lg:min-w-900 p-5">
          ...
          <div className="flex flex-col items-start md:items-center justify-center md:flex-row rounded-b-lg bg-white-50%">
            ...
            <form
              className="flex w-full flex-col p-3 items-start md:items-center justify-center"
              onSubmit={onSubmit}
            >
